<template>
    <div>
        ...
        <nav-bread>
            <span slot="bread" style="font-weight:bold;">订单确认</span>
        </nav-bread>
        <div>
                ...
                <div class="checkout-order">
                    ...
                    <div class="order-foot-wrap">
                        <div class="prev-btn-wrap">
                            <router-link to="/address" class="btn btn--m">返回>></router-link>
                        </div>
                        <div class="next-btn-wrap">
                            <button class="btn btn--m btn--red" @click="payMent">订单支付</button>
                        </div>
                    </div>
                </div>
            </div>
        ...
        methods:{
            init(){
                ...
                axios.get("/users/cartList").then((response)=>{
                    ...
                    //虽然复用购物车列表但是我们在订单中只展示选中的商品（v-if="item.checked==='1'"）
            ...
            payMent(){
                ...
                axios.post("users/payMent",{
                    ...
                    let res=response.data;
                    if(res.status=="0"){
                        console.log("order creates successfully");
                        //跳转到订单成功页面,但是由于是hash，这些参数以哈希值传输
                        let param = res.result.orderId;
                        this.$router.push({path:"/orderSuccess?orderId="+param});
                    }
                })
            },
        }
