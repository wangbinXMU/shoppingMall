    ...
    <header class="header">
        ...
        <div class="navbar">
            <div class="navbar-left-container">
                <a href="/">
                    <!--logo图片-->
                    <img class="navbar-brand-logo" src="/static/logo.bmp">
                </a>
            </div>
            <div class="navbar-right-container" style="display: flex;">
                <div class="navbar-menu-container">
                    ...
                    <div class="navbar-cart-container">
                        <!--购物车标记数量显示，只有当购物车中的数量大于0才显示数量标记-->
...
export default {
  ...
  computed: {
    ...
    nickName: {
      ...
      // setter
      set: function(tempName) {
        this.$store.state.nickName = tempName;
      }
    },
    // 购物车商品数量
    cartCount: {
      ...
      // setter
      set: function(tempName) {
        this.$store.state.cartCount = tempName;
      }
    }
  },
  mounted() {
  ...
  methods: {
    ...
    login() {
      ...
      axios
        ...
        .then(response => {
          // 后端返回的是用户名，并将userId和username保存在前端cookie
          // 故判断是否处于登录状态主要看cookie是否有值
          let res = response.data;
          if (res.status == "0") {
            this.errorTip = false;
    ...
    getCartCount() {
      axios.get("/users/getTotal").then(response => {
        let res = response.data;
        // 将后台获取的商品数量传给vuex去更新右上角的数量标记
